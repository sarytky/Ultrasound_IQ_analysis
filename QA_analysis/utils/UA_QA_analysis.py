...
def crop_US_im(im, crop2half=True):
    ...
    loc = np.argmax(sizes)
    if (loc == 0).all():  # if nothing is found from the upper image then assign loc to 1
        loc = 1
    ...
    y_vals = vals[0]
    if (y_vals == 10).any():  # Tenth pixel is still image header  -->border is the largest component so the next biggest component is the actual image regions
        sizes[loc] = 0
    ...
    if crop2half:  # Reverberations are present in the upper half of the ultrasound image
